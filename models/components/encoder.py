...
class RepDSConv(nn.Module):
    """
    Reparameterizable Depthwise Separable Convolution block.
    During training: multi-branch depthwise conv + standard pointwise conv.
    During inference: fused single depthwise conv + standard pointwise conv.
    """
    def __init__(self, in_channels, out_channels, deploy=False):
        ...
        if deploy:
            self.dw_reparam = nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1, groups=in_channels, bias=True)
    ...
    def switch_to_deploy(self):
        ...
        # Instantiate the single fused 3x3 DW convolution
    ...
    def get_equivalent_kernel_bias(self):
        # 1. Fuse the 3x3 Conv + BN branch
        kernel_3x3, bias_3x3 = self._fuse_conv_bn(self.dw_3x3, self.bn_3x3)

        # 2. Fuse the 1x1 Conv + BN branch and pad kernel to 3x3
        kernel_1x1_raw, bias_1x1 = self._fuse_conv_bn(self.dw_1x1, self.bn_1x1)
        kernel_1x1 = F.pad(kernel_1x1_raw, [1, 1, 1, 1])

        # 3. Fuse the Identity (BN only) branch
        # Create identity 3x3 depthwise kernel: shape [in_channels, 1, 3, 3]
        device = self.bn_identity.weight.device
        id_kernel = torch.zeros((self.in_channels, 1, 3, 3), device=device)
        for i in range(self.in_channels):
            id_kernel[i, 0, 1, 1] = 1.0
            
        kernel_id, bias_id = self._fuse_conv_bn_raw(id_kernel, None, self.bn_identity)

        # 4. Sum up all fused branches
